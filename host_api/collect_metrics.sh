...
# Note: This runs on the host (WSL1 or native Linux), so it reads real host metrics
# from /proc, /sys, etc. The script will set its own SCRIPT_DIR and derive
# DATA_DIR, LOG_DIR, REPORT_DIR from it
...
# State file to persist previous values between API calls
# This is needed for CPU usage and network speed delta calculations
STATE_DIR="${DATA_DIR:-${SCRIPT_DIR}/../data}"
mkdir -p "$STATE_DIR" 2>/dev/null || true
STATE_FILE="${STATE_DIR}/metrics_state.txt"

# Initialize global state (required for delta calculations)
# These will be loaded from state file if it exists
PREV_CPU_TOTAL=0
PREV_CPU_IDLE=0
PREV_NET_TIME=0
...
# Load previous state if it exists (for CPU and network delta calculations)
if [ -f "$STATE_FILE" ]; then
    # Read state file line by line and restore values
    while IFS= read -r line; do
        # Skip empty lines and comments
        [[ -z "$line" || "$line" =~ ^# ]] && continue
        
        # Parse key=value or key[index]=value
        if [[ "$line" =~ ^PREV_CPU_TOTAL=(.+)$ ]]; then
            PREV_CPU_TOTAL="${BASH_REMATCH[1]}"
        elif [[ "$line" =~ ^PREV_CPU_IDLE=(.+)$ ]]; then
            PREV_CPU_IDLE="${BASH_REMATCH[1]}"
        elif [[ "$line" =~ ^PREV_NET_TIME=(.+)$ ]]; then
            PREV_NET_TIME="${BASH_REMATCH[1]}"
        elif [[ "$line" =~ ^PREV_NET_RX\[(.+)\]=(.+)$ ]]; then
            interface="${BASH_REMATCH[1]}"
            value="${BASH_REMATCH[2]}"
            PREV_NET_RX["$interface"]="$value"
        elif [[ "$line" =~ ^PREV_NET_TX\[(.+)\]=(.+)$ ]]; then
            interface="${BASH_REMATCH[1]}"
            value="${BASH_REMATCH[2]}"
            PREV_NET_TX["$interface"]="$value"
        fi
...
# Save state for next call (for CPU and network delta calculations)
# This allows CPU usage and network speed to be calculated correctly
{
    echo "PREV_CPU_TOTAL=${PREV_CPU_TOTAL}"
    echo "PREV_CPU_IDLE=${PREV_CPU_IDLE}"
    echo "PREV_NET_TIME=${PREV_NET_TIME}"
    # Export associative arrays (network state)
    for key in "${!PREV_NET_RX[@]}"; do
        echo "PREV_NET_RX[${key}]=${PREV_NET_RX[$key]}"
    done
    for key in "${!PREV_NET_TX[@]}"; do
        echo "PREV_NET_TX[${key}]=${PREV_NET_TX[$key]}"
    done
